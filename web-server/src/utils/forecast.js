...
const forecast = (lat, lon, callback) => {
  ...
  request({ url, json: true }, (reject, response) => {
    ...
    if (reject) {
      ...
      callback(
        undefined,
        `${current.weather_descriptions}. It is currently ${current.temperature} degrees out. It feels like ${current.feelslike} degrees out. The humidity is ${current.humidity}%.`
      );
    }
  });
};
